...
describe("ProductEntity", function()
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set CODATPLATFORM_TEST_PRODUCT_ENTID JSON to run live")
      return
    end
    local client = setup.client

    -- Bootstrap entity data from existing test data.
    local product_ref01_data_raw = vs.items(helpers.to_map(
      vs.getpath(setup.data, "existing.product")))
    local product_ref01_data = nil
    if #product_ref01_data_raw > 0 then
...
function product_basic_setup(extra)
  ...
  local idmap = vs.transform(
    { "product01", "product02", "product03", "company01", "company02", "company03" },
    {
      ["`$PACK`"] = { "", {
        ["`$KEY`"] = "`$COPY`",
  ...
  local entid_env_raw = os.getenv("CODATPLATFORM_TEST_PRODUCT_ENTID")
  ...
  local idmap_resolved = helpers.to_map(
    env["CODATPLATFORM_TEST_PRODUCT_ENTID"])
