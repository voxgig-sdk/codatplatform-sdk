...
describe("ValidationDirect", function()
  it("should direct-list-validation", function()
    local setup = validation_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-validation", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
    if setup.live then
      for _, _live_key in ipairs({"company01", "sync01"}) do
...
function validation_direct_setup(mockres)
