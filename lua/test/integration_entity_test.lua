...
describe("IntegrationEntity", function()
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set CODATPLATFORM_TEST_INTEGRATION_ENTID JSON to run live")
      return
    end
    local client = setup.client

    -- Bootstrap entity data from existing test data.
    local integration_ref01_data_raw = vs.items(helpers.to_map(
      vs.getpath(setup.data, "existing.integration")))
    local integration_ref01_data = nil
    if #integration_ref01_data_raw > 0 then
      integration_ref01_data = helpers.to_map(integration_ref01_data_raw[1][2])
    end
...
function integration_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("CODATPLATFORM_TEST_INTEGRATION_ENTID")
  ...
  local idmap_resolved = helpers.to_map(
    env["CODATPLATFORM_TEST_INTEGRATION_ENTID"])
