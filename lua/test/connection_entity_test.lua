...
describe("ConnectionEntity", function()
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set CODATPLATFORM_TEST_CONNECTION_ENTID JSON to run live")
      return
    end
    ...
    local connection_ref01_data = helpers.to_map(vs.getprop(
    ...
    -- UPDATE
    local connection_ref01_data_up0_up = {
      id = connection_ref01_data["id"],
      ["company_id"] = setup.idmap["company_id"],
    }

    local connection_ref01_markdef_up0_name = "created"
    local connection_ref01_markdef_up0_value = "Mark01-connection_ref01_" .. tostring(setup.now)
    ...
    -- LOAD
    local connection_ref01_match_dt0 = {
      id = connection_ref01_data["id"],
    }
    ...
    -- REMOVE
    local connection_ref01_match_rm0 = {
      id = connection_ref01_data["id"],
    }
    local _, err = connection_ref01_ent:remove(connection_ref01_match_rm0, nil)
    assert.is_nil(err)

    -- LIST
    local connection_ref01_match_rt0 = {
      ["company_id"] = setup.idmap["company01"],
    }

    local connection_ref01_list_rt0_result, err = connection_ref01_ent:list(connection_ref01_match_rt0, nil)
    assert.is_nil(err)
    assert.is_table(connection_ref01_list_rt0_result)
...
function connection_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("CODATPLATFORM_TEST_CONNECTION_ENTID")
  ...
  if idmap_resolved["company_id"] == nil then
