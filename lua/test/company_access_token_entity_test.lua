...
describe("CompanyAccessTokenEntity", function()
  it("should create instance", function()
    local testsdk = sdk.test(nil, nil)
    local ent = testsdk:CompanyAccessToken(nil)
    assert.is_not_nil(ent)
  end)

  it("should run basic flow", function()
    local setup = company_access_token_basic_setup(nil)
    -- Per-op sdk-test-control.json skip.
    local _live = setup.live or false
    for _, _op in ipairs({"load"}) do
      local _should_skip, _reason = runner.is_control_skipped("entityOp", "company_access_token." .. _op, _live and "live" or "unit")
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set CODATPLATFORM_TEST_COMPANY_ACCESS_TOKEN_ENTID JSON to run live")
      return
    end
    local client = setup.client

    -- Bootstrap entity data from existing test data.
    local company_access_token_ref01_data_raw = vs.items(helpers.to_map(
      vs.getpath(setup.data, "existing.company_access_token")))
    local company_access_token_ref01_data = nil
    if #company_access_token_ref01_data_raw > 0 then
      company_access_token_ref01_data = helpers.to_map(company_access_token_ref01_data_raw[1][2])
    end
...
function company_access_token_basic_setup(extra)
  runner.load_env_local()

  local entity_data_file = _test_dir .. "../../.sdk/test/entity/company_access_token/CompanyAccessTokenTestData.json"
  local f = io.open(entity_data_file, "r")
  if f == nil then
    error("failed to read company_access_token test data: " .. entity_data_file)
  end
  ...
  local entid_env_raw = os.getenv("CODATPLATFORM_TEST_COMPANY_ACCESS_TOKEN_ENTID")
