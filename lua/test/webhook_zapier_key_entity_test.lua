...
describe("WebhookZapierKeyEntity", function()
  it("should create instance", function()
    local testsdk = sdk.test(nil, nil)
    local ent = testsdk:WebhookZapierKey(nil)
    assert.is_not_nil(ent)
  end)

  it("should run basic flow", function()
    local setup = webhook_zapier_key_basic_setup(nil)
    -- Per-op sdk-test-control.json skip.
    local _live = setup.live or false
    for _, _op in ipairs({"create"}) do
      local _should_skip, _reason = runner.is_control_skipped("entityOp", "webhook_zapier_key." .. _op, _live and "live" or "unit")
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set CODATPLATFORM_TEST_WEBHOOK_ZAPIER_KEY_ENTID JSON to run live")
      return
    end
    local client = setup.client

    -- CREATE
    local webhook_zapier_key_ref01_ent = client:WebhookZapierKey(nil)
    local webhook_zapier_key_ref01_data = helpers.to_map(vs.getprop(
...
function webhook_zapier_key_basic_setup(extra)
  ...
  local entity_data_file = _test_dir .. "../../.sdk/test/entity/webhook_zapier_key/WebhookZapierKeyTestData.json"
  local f = io.open(entity_data_file, "r")
  if f == nil then
    error("failed to read webhook_zapier_key test data: " .. entity_data_file)
  end
  ...
  local entid_env_raw = os.getenv("CODATPLATFORM_TEST_WEBHOOK_ZAPIER_KEY_ENTID")
