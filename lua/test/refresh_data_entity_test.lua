...
describe("RefreshDataEntity", function()
  it("should create instance", function()
    local testsdk = sdk.test(nil, nil)
    local ent = testsdk:RefreshData(nil)
    assert.is_not_nil(ent)
  end)

  it("should run basic flow", function()
    local setup = refresh_data_basic_setup(nil)
    -- Per-op sdk-test-control.json skip.
    local _live = setup.live or false
    for _, _op in ipairs({"create"}) do
      local _should_skip, _reason = runner.is_control_skipped("entityOp", "refresh_data." .. _op, _live and "live" or "unit")
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set CODATPLATFORM_TEST_REFRESH_DATA_ENTID JSON to run live")
      return
    end
    local client = setup.client

    -- CREATE
    local refresh_data_ref01_ent = client:RefreshData(nil)
    local refresh_data_ref01_data = helpers.to_map(vs.getprop(
...
function refresh_data_basic_setup(extra)
  runner.load_env_local()

  local entity_data_file = _test_dir .. "../../.sdk/test/entity/refresh_data/RefreshDataTestData.json"
  local f = io.open(entity_data_file, "r")
  if f == nil then
    error("failed to read refresh_data test data: " .. entity_data_file)
  end
  ...
  local entid_env_raw = os.getenv("CODATPLATFORM_TEST_REFRESH_DATA_ENTID")
