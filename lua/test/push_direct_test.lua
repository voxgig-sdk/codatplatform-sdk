...
describe("PushDirect", function()
  it("should direct-list-push", function()
    local setup = push_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-push", setup.live and "live" or "unit")
  ...
  it("should direct-load-push", function()
    local setup = push_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-push", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function push_direct_setup(mockres)
