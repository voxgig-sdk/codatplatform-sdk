...
describe("CompanyEntity", function()
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set CODATPLATFORM_TEST_COMPANY_ENTID JSON to run live")
      return
    end
    ...
    local company_ref01_data = helpers.to_map(vs.getprop(
    ...
    -- UPDATE
    local company_ref01_data_up0_up = {
      id = company_ref01_data["id"],
    }

    local company_ref01_markdef_up0_name = "created"
    local company_ref01_markdef_up0_value = "Mark01-company_ref01_" .. tostring(setup.now)
    ...
    -- REMOVE
    local company_ref01_match_rm0 = {
      id = company_ref01_data["id"],
    }
    local _, err = company_ref01_ent:remove(company_ref01_match_rm0, nil)
    assert.is_nil(err)

    -- LIST
    local company_ref01_match_rt0 = {}

    local company_ref01_list_rt0_result, err = company_ref01_ent:list(company_ref01_match_rt0, nil)
    assert.is_nil(err)
    assert.is_table(company_ref01_list_rt0_result)
...
function company_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("CODATPLATFORM_TEST_COMPANY_ENTID")
