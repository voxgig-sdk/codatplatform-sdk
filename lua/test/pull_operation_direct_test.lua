...
describe("PullOperationDirect", function()
  it("should direct-list-pull_operation", function()
    local setup = pull_operation_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-pull_operation", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
    if setup.live then
      for _, _live_key in ipairs({"company01"}) do
  ...
  it("should direct-load-pull_operation", function()
    local setup = pull_operation_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-pull_operation", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function pull_operation_direct_setup(mockres)
