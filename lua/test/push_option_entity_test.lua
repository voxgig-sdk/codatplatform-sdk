...
describe("PushOptionEntity", function()
  it("should create instance", function()
    local testsdk = sdk.test(nil, nil)
    local ent = testsdk:PushOption(nil)
    assert.is_not_nil(ent)
  end)

  it("should run basic flow", function()
    local setup = push_option_basic_setup(nil)
    -- Per-op sdk-test-control.json skip.
    local _live = setup.live or false
    for _, _op in ipairs({"load"}) do
      local _should_skip, _reason = runner.is_control_skipped("entityOp", "push_option." .. _op, _live and "live" or "unit")
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set CODATPLATFORM_TEST_PUSH_OPTION_ENTID JSON to run live")
      return
    end
    local client = setup.client

    -- Bootstrap entity data from existing test data.
    local push_option_ref01_data_raw = vs.items(helpers.to_map(
      vs.getpath(setup.data, "existing.push_option")))
    local push_option_ref01_data = nil
    if #push_option_ref01_data_raw > 0 then
      push_option_ref01_data = helpers.to_map(push_option_ref01_data_raw[1][2])
    end
...
function push_option_basic_setup(extra)
  ...
  local entity_data_file = _test_dir .. "../../.sdk/test/entity/push_option/PushOptionTestData.json"
  local f = io.open(entity_data_file, "r")
  if f == nil then
    error("failed to read push_option test data: " .. entity_data_file)
  end
  ...
  local entid_env_raw = os.getenv("CODATPLATFORM_TEST_PUSH_OPTION_ENTID")
