-- Codatplatform SDK AccessToken entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local AccessTokenEntity = {}
AccessTokenEntity.__index = AccessTokenEntity


function AccessTokenEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, AccessTokenEntity)
  self._name = "access_token"
  self._client = client
...
function AccessTokenEntity:get_name()
  return self._name
end


function AccessTokenEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return AccessTokenEntity.new(self._client, opts)
end
...
function AccessTokenEntity:mark_deleted()
  self._deleted = true
end


function AccessTokenEntity:deleted()
  return true == self._deleted
end


function AccessTokenEntity:data_set(args)
  if args ~= nil then
...
function AccessTokenEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function AccessTokenEntity:match_set(args)
  if args ~= nil then
...
function AccessTokenEntity:match_get()
...
function AccessTokenEntity:stream(action, args, callopts)
...
function AccessTokenEntity:_run_op(ctx, post_done)
  local utility = self._utility
