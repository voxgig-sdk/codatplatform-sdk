-- Codatplatform SDK DataType entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local DataTypeEntity = {}
DataTypeEntity.__index = DataTypeEntity


function DataTypeEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, DataTypeEntity)
...
function DataTypeEntity:get_name()
  return self._name
end


function DataTypeEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return DataTypeEntity.new(self._client, opts)
end
...
function DataTypeEntity:mark_deleted()
  self._deleted = true
end


function DataTypeEntity:deleted()
  return true == self._deleted
end


function DataTypeEntity:data_set(args)
  if args ~= nil then
...
function DataTypeEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function DataTypeEntity:match_set(args)
  if args ~= nil then
...
function DataTypeEntity:match_get()
...
function DataTypeEntity:stream(action, args, callopts)
...
function DataTypeEntity:_run_op(ctx, post_done)
  local utility = self._utility
