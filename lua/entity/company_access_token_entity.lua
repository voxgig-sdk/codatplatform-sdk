...
local CompanyAccessTokenEntity = {}
CompanyAccessTokenEntity.__index = CompanyAccessTokenEntity


function CompanyAccessTokenEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, CompanyAccessTokenEntity)
  self._name = "company_access_token"
  self._client = client
...
function CompanyAccessTokenEntity:get_name()
  return self._name
end


function CompanyAccessTokenEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CompanyAccessTokenEntity.new(self._client, opts)
end
...
function CompanyAccessTokenEntity:mark_deleted()
  self._deleted = true
end


function CompanyAccessTokenEntity:deleted()
  return true == self._deleted
end


function CompanyAccessTokenEntity:data_set(args)
  if args ~= nil then
...
function CompanyAccessTokenEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CompanyAccessTokenEntity:match_set(args)
  if args ~= nil then
...
function CompanyAccessTokenEntity:match_get()
...
function CompanyAccessTokenEntity:stream(action, args, callopts)
...
---@param reqmatch CompanyAccessTokenLoadMatch
---@param ctrl? table
---@return CompanyAccessToken
---@return string? err
function CompanyAccessTokenEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function CompanyAccessTokenEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return CompanyAccessTokenEntity
