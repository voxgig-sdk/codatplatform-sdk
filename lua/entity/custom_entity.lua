...
function CustomEntity.new(client, entopts)
  entopts = entopts or {}
...
function CustomEntity:get_name()
  return self._name
end


function CustomEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CustomEntity.new(self._client, opts)
end
...
function CustomEntity:mark_deleted()
  self._deleted = true
end


function CustomEntity:deleted()
  return true == self._deleted
end


function CustomEntity:data_set(args)
  if args ~= nil then
...
function CustomEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CustomEntity:match_set(args)
  if args ~= nil then
...
function CustomEntity:match_get()
...
function CustomEntity:stream(action, args, callopts)
...
function CustomEntity:_run_op(ctx, post_done)
  local utility = self._utility
