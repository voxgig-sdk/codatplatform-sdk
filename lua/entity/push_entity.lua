...
local PushEntity = {}
PushEntity.__index = PushEntity


function PushEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, PushEntity)
  self._name = "push"
  self._client = client
...
function PushEntity:get_name()
  return self._name
end


function PushEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return PushEntity.new(self._client, opts)
end
...
function PushEntity:mark_deleted()
  self._deleted = true
end


function PushEntity:deleted()
  return true == self._deleted
end


function PushEntity:data_set(args)
  if args ~= nil then
...
function PushEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function PushEntity:match_set(args)
  if args ~= nil then
...
function PushEntity:stream(action, args, callopts)
...
---@param reqmatch PushLoadMatch
---@param ctrl? table
---@return Push
---@return string? err
function PushEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch PushListMatch
---@param ctrl? table
---@return Push[]
---@return string? err
function PushEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
function PushEntity:_run_op(ctx, post_done)
  local utility = self._utility
