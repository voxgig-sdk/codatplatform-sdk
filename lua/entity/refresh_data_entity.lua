...
local RefreshDataEntity = {}
RefreshDataEntity.__index = RefreshDataEntity


function RefreshDataEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, RefreshDataEntity)
  self._name = "refresh_data"
  self._client = client
...
function RefreshDataEntity:get_name()
  return self._name
end


function RefreshDataEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return RefreshDataEntity.new(self._client, opts)
end
...
function RefreshDataEntity:mark_deleted()
  self._deleted = true
end


function RefreshDataEntity:deleted()
  return true == self._deleted
end


function RefreshDataEntity:data_set(args)
  if args ~= nil then
...
function RefreshDataEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function RefreshDataEntity:match_set(args)
  if args ~= nil then
...
function RefreshDataEntity:match_get()
...
function RefreshDataEntity:stream(action, args, callopts)
...
---@param reqdata RefreshDataCreateData
---@param ctrl? table
---@return RefreshData
---@return string? err
function RefreshDataEntity:create(reqdata, ctrl)
  local utility = self._utility
...
function RefreshDataEntity:_run_op(ctx, post_done)
  local utility = self._utility
