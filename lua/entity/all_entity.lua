-- Codatplatform SDK All entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local AllEntity = {}
AllEntity.__index = AllEntity


function AllEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, AllEntity)
  self._name = "all"
  self._client = client
...
function AllEntity:get_name()
  return self._name
end


function AllEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return AllEntity.new(self._client, opts)
end
...
function AllEntity:mark_deleted()
  self._deleted = true
end


function AllEntity:deleted()
  return true == self._deleted
end


function AllEntity:data_set(args)
  if args ~= nil then
...
function AllEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function AllEntity:match_set(args)
  if args ~= nil then
...
function AllEntity:stream(action, args, callopts)
...
function AllEntity:_run_op(ctx, post_done)
  local utility = self._utility
