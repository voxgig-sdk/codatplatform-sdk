-- Codatplatform SDK Option entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local OptionEntity = {}
OptionEntity.__index = OptionEntity


function OptionEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, OptionEntity)
  self._name = "option"
  self._client = client
...
function OptionEntity:get_name()
  return self._name
end


function OptionEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return OptionEntity.new(self._client, opts)
end
...
function OptionEntity:mark_deleted()
  self._deleted = true
end


function OptionEntity:deleted()
  return true == self._deleted
end


function OptionEntity:data_set(args)
  if args ~= nil then
...
function OptionEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function OptionEntity:match_set(args)
  if args ~= nil then
...
function OptionEntity:match_get()
...
function OptionEntity:stream(action, args, callopts)
...
function OptionEntity:_run_op(ctx, post_done)
  local utility = self._utility
