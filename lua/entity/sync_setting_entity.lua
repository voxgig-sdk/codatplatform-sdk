...
local SyncSettingEntity = {}
SyncSettingEntity.__index = SyncSettingEntity


function SyncSettingEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, SyncSettingEntity)
  self._name = "sync_setting"
  self._client = client
...
function SyncSettingEntity:get_name()
  return self._name
end


function SyncSettingEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return SyncSettingEntity.new(self._client, opts)
end
...
function SyncSettingEntity:mark_deleted()
  self._deleted = true
end


function SyncSettingEntity:deleted()
  return true == self._deleted
end


function SyncSettingEntity:data_set(args)
  if args ~= nil then
...
function SyncSettingEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function SyncSettingEntity:match_set(args)
  if args ~= nil then
...
function SyncSettingEntity:match_get()
...
function SyncSettingEntity:stream(action, args, callopts)
...
---@param reqmatch SyncSettingListMatch
---@param ctrl? table
---@return SyncSetting[]
---@return string? err
function SyncSettingEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function SyncSettingEntity:_run_op(ctx, post_done)
  local utility = self._utility
