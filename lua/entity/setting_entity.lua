...
local SettingEntity = {}
SettingEntity.__index = SettingEntity


function SettingEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, SettingEntity)
  self._name = "setting"
  self._client = client
...
function SettingEntity:get_name()
  return self._name
end


function SettingEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return SettingEntity.new(self._client, opts)
end
...
function SettingEntity:mark_deleted()
  self._deleted = true
end


function SettingEntity:deleted()
  return true == self._deleted
end


function SettingEntity:data_set(args)
  if args ~= nil then
...
function SettingEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function SettingEntity:match_set(args)
  if args ~= nil then
...
function SettingEntity:match_get()
...
function SettingEntity:stream(action, args, callopts)
...
---@param reqmatch SettingListMatch
---@param ctrl? table
---@return Setting[]
---@return string? err
function SettingEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqdata SettingCreateData
---@param ctrl? table
---@return Setting
---@return string? err
function SettingEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
---@param reqmatch SettingRemoveMatch
---@param ctrl? table
---@return Setting
---@return string? err
function SettingEntity:remove(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "remove",
    ctrl = ctrl,
...
function SettingEntity:_run_op(ctx, post_done)
  local utility = self._utility
