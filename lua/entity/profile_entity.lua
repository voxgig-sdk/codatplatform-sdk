...
local ProfileEntity = {}
ProfileEntity.__index = ProfileEntity


function ProfileEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ProfileEntity)
  self._name = "profile"
  self._client = client
...
function ProfileEntity:get_name()
  return self._name
end


function ProfileEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ProfileEntity.new(self._client, opts)
end
...
function ProfileEntity:mark_deleted()
  self._deleted = true
end


function ProfileEntity:deleted()
  return true == self._deleted
end


function ProfileEntity:data_set(args)
  if args ~= nil then
...
function ProfileEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ProfileEntity:match_set(args)
  if args ~= nil then
...
function ProfileEntity:match_get()
...
function ProfileEntity:stream(action, args, callopts)
...
---@param reqmatch ProfileListMatch
---@param ctrl? table
---@return Profile[]
---@return string? err
function ProfileEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqdata ProfileUpdateData
---@param ctrl? table
---@return Profile
---@return string? err
function ProfileEntity:update(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "update",
    ctrl = ctrl,
...
function ProfileEntity:_run_op(ctx, post_done)
  local utility = self._utility
