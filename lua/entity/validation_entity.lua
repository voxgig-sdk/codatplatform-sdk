...
local ValidationEntity = {}
ValidationEntity.__index = ValidationEntity


function ValidationEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ValidationEntity)
...
function ValidationEntity:get_name()
  return self._name
end


function ValidationEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ValidationEntity.new(self._client, opts)
end
...
function ValidationEntity:mark_deleted()
  self._deleted = true
end


function ValidationEntity:deleted()
  return true == self._deleted
end


function ValidationEntity:data_set(args)
  if args ~= nil then
...
function ValidationEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ValidationEntity:match_set(args)
  if args ~= nil then
...
function ValidationEntity:match_get()
...
function ValidationEntity:stream(action, args, callopts)
...
---@param reqmatch ValidationListMatch
---@param ctrl? table
---@return Validation[]
---@return string? err
function ValidationEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function ValidationEntity:_run_op(ctx, post_done)
  local utility = self._utility
