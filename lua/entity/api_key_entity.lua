-- Codatplatform SDK ApiKey entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local ApiKeyEntity = {}
ApiKeyEntity.__index = ApiKeyEntity


function ApiKeyEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ApiKeyEntity)
  self._name = "api_key"
  self._client = client
...
function ApiKeyEntity:get_name()
  return self._name
end


function ApiKeyEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ApiKeyEntity.new(self._client, opts)
end
...
function ApiKeyEntity:mark_deleted()
  self._deleted = true
end


function ApiKeyEntity:deleted()
  return true == self._deleted
end


function ApiKeyEntity:data_set(args)
  if args ~= nil then
...
function ApiKeyEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ApiKeyEntity:match_set(args)
  if args ~= nil then
...
function ApiKeyEntity:match_get()
...
function ApiKeyEntity:stream(action, args, callopts)
...
function ApiKeyEntity:_run_op(ctx, post_done)
  local utility = self._utility
