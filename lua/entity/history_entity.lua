-- Codatplatform SDK History entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local HistoryEntity = {}
HistoryEntity.__index = HistoryEntity


function HistoryEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, HistoryEntity)
  self._name = "history"
  self._client = client
...
function HistoryEntity:get_name()
  return self._name
end


function HistoryEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return HistoryEntity.new(self._client, opts)
end
...
function HistoryEntity:mark_deleted()
  self._deleted = true
end


function HistoryEntity:deleted()
  return true == self._deleted
end


function HistoryEntity:data_set(args)
  if args ~= nil then
...
function HistoryEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function HistoryEntity:match_set(args)
  if args ~= nil then
...
function HistoryEntity:match_get()
...
function HistoryEntity:stream(action, args, callopts)
...
function HistoryEntity:_run_op(ctx, post_done)
  local utility = self._utility
