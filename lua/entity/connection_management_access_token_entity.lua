...
local ConnectionManagementAccessTokenEntity = {}
ConnectionManagementAccessTokenEntity.__index = ConnectionManagementAccessTokenEntity


function ConnectionManagementAccessTokenEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ConnectionManagementAccessTokenEntity)
  self._name = "connection_management_access_token"
  self._client = client
...
function ConnectionManagementAccessTokenEntity:get_name()
  return self._name
end


function ConnectionManagementAccessTokenEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ConnectionManagementAccessTokenEntity.new(self._client, opts)
end
...
function ConnectionManagementAccessTokenEntity:mark_deleted()
  self._deleted = true
end


function ConnectionManagementAccessTokenEntity:deleted()
  return true == self._deleted
end


function ConnectionManagementAccessTokenEntity:data_set(args)
  if args ~= nil then
...
function ConnectionManagementAccessTokenEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ConnectionManagementAccessTokenEntity:match_set(args)
  if args ~= nil then
...
function ConnectionManagementAccessTokenEntity:match_get()
...
function ConnectionManagementAccessTokenEntity:stream(action, args, callopts)
...
---@param reqmatch ConnectionManagementAccessTokenLoadMatch
---@param ctrl? table
---@return ConnectionManagementAccessToken
---@return string? err
function ConnectionManagementAccessTokenEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function ConnectionManagementAccessTokenEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return ConnectionManagementAccessTokenEntity
