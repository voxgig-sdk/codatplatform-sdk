-- Codatplatform SDK Product entity
...
function ProductEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ProductEntity)
  self._name = "product"
  self._client = client
...
function ProductEntity:get_name()
  return self._name
end


function ProductEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ProductEntity.new(self._client, opts)
end
...
function ProductEntity:mark_deleted()
  self._deleted = true
end


function ProductEntity:deleted()
  return true == self._deleted
end


function ProductEntity:data_set(args)
  if args ~= nil then
...
function ProductEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ProductEntity:match_set(args)
  if args ~= nil then
...
function ProductEntity:match_get()
...
function ProductEntity:stream(action, args, callopts)
...
function ProductEntity:_run_op(ctx, post_done)
  local utility = self._utility
