-- Codatplatform SDK Queue entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local QueueEntity = {}
QueueEntity.__index = QueueEntity


function QueueEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, QueueEntity)
  self._name = "queue"
  self._client = client
...
function QueueEntity:get_name()
  return self._name
end


function QueueEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return QueueEntity.new(self._client, opts)
end
...
function QueueEntity:mark_deleted()
  self._deleted = true
end


function QueueEntity:deleted()
  return true == self._deleted
end


function QueueEntity:data_set(args)
  if args ~= nil then
...
function QueueEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function QueueEntity:match_set(args)
  if args ~= nil then
...
function QueueEntity:match_get()
...
function QueueEntity:stream(action, args, callopts)
...
function QueueEntity:_run_op(ctx, post_done)
  local utility = self._utility
