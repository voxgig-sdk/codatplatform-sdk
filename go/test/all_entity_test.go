...
func TestAllEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set CODATPLATFORM_TEST_ALL_ENTID JSON to run live")
			return
		}
		// Bootstrap entity data from existing test data (no create step in flow).
		allRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.all")))
...
func allBasicSetup(extra map[string]any) *entityTestSetup {
	...
	idmap, _ := vs.Transform(
		[]any{"all01", "all02", "all03", "company01", "company02", "company03"},
		map[string]any{
	...
	entidEnvRaw := os.Getenv("CODATPLATFORM_TEST_ALL_ENTID")
	...
	idmapResolved := core.ToMapAny(env["CODATPLATFORM_TEST_ALL_ENTID"])
