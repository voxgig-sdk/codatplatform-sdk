...
func TestPushDirect(t *testing.T) {
	t.Run("direct-list-push", func(t *testing.T) {
		setup := pushDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-push", _mode); _shouldSkip {
			if _reason == "" {
	...
	t.Run("direct-load-push", func(t *testing.T) {
		setup := pushDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-push", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
...
type pushDirectSetupResult struct {
...
func pushDirectSetup(mockres any) *pushDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"CODATPLATFORM_TEST_PUSH_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["CODATPLATFORM_TEST_PUSH_ENTID"]; ok {
		...
		return &pushDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &pushDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
