...
func TestCustomDirect(t *testing.T) {
	t.Run("direct-load-custom", func(t *testing.T) {
		setup := customDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-custom", _mode); _shouldSkip {
			if _reason == "" {
...
type customDirectSetupResult struct {
...
func customDirectSetup(mockres any) *customDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"CODATPLATFORM_TEST_CUSTOM_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["CODATPLATFORM_TEST_CUSTOM_ENTID"]; ok {
		...
		return &customDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &customDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
