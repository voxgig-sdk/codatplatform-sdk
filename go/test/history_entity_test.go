...
func TestHistoryEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		historyRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.history")))
...
func historyBasicSetup(extra map[string]any) *entityTestSetup {
	...
	idmap, _ := vs.Transform(
		[]any{"history01", "history02", "history03", "company01", "company02", "company03"},
		map[string]any{
	...
	entidEnvRaw := os.Getenv("CODATPLATFORM_TEST_HISTORY_ENTID")
	...
	idmapResolved := core.ToMapAny(env["CODATPLATFORM_TEST_HISTORY_ENTID"])
