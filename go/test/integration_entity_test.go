...
func TestIntegrationEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set CODATPLATFORM_TEST_INTEGRATION_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func integrationBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("CODATPLATFORM_TEST_INTEGRATION_ENTID")
	...
	idmapResolved := core.ToMapAny(env["CODATPLATFORM_TEST_INTEGRATION_ENTID"])
