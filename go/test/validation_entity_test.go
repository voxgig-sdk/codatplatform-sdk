...
func TestValidationEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.Validation(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.Validation(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set CODATPLATFORM_TEST_VALIDATION_ENTID JSON to run live")
			return
		}
		client := setup.client

		// Bootstrap entity data from existing test data (no create step in flow).
		validationRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.validation")))
		...
		validationRef01Match := map[string]any{
			"company_id": setup.idmap["company01"],
			"sync_id": setup.idmap["sync01"],
		}

		validationRef01ListResult, err := validationRef01Ent.List(validationRef01Match, nil)
		if err != nil {
			t.Fatalf("list failed: %v", err)
		}
		_, validationRef01ListOk := validationRef01ListResult.([]any)
		if !validationRef01ListOk {
			t.Fatalf("expected list result to be an array, got %T", validationRef01ListResult)
		}
...
func validationBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("CODATPLATFORM_TEST_VALIDATION_ENTID")
	...
	idmapResolved := core.ToMapAny(env["CODATPLATFORM_TEST_VALIDATION_ENTID"])
