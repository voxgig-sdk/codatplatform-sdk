...
import (
	...
	omni "github.com/voxgig-sdk/codatplatform-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.CodatplatformError); is {
				out := map[string]any{
					"name":    "CodatplatformError",
...
func omniCtx(arg any, client *sdk.CodatplatformSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.CodatplatformSDK); is {
			client = live
