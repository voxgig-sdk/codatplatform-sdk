...
func TestProductEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		productRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.product")))
...
func productBasicSetup(extra map[string]any) *entityTestSetup {
	...
	idmap, _ := vs.Transform(
		[]any{"product01", "product02", "product03", "company01", "company02", "company03"},
		map[string]any{
	...
	entidEnvRaw := os.Getenv("CODATPLATFORM_TEST_PRODUCT_ENTID")
	...
	idmapResolved := core.ToMapAny(env["CODATPLATFORM_TEST_PRODUCT_ENTID"])
