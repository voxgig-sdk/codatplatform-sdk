...
from codatplatform_sdk.codatplatform_types import (
...
class ConnectionManagementAllowedOriginEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return ConnectionManagementAllowedOriginEntity(self._client, opts)
    ...
    def data_get(self) -> ConnectionManagementAllowedOrigin:
    ...
    def match_get(self) -> ConnectionManagementAllowedOrigin:
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[ConnectionManagementAllowedOrigin]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.ConnectionManagementAllowedOrigin().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def create(self, reqdata: ConnectionManagementAllowedOriginCreateData, ctrl=None) -> ConnectionManagementAllowedOrigin:
        utility = self._utility
        ctx = utility.make_context({
            "opname": "create",
            "ctrl": ctrl,
