...
class SyncSettingEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return SyncSettingEntity(self._client, opts)
    ...
    def match_get(self) -> SyncSetting:
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[SyncSetting]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.SyncSetting().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
