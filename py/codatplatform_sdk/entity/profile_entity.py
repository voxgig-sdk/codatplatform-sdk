...
class ProfileEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return ProfileEntity(self._client, opts)
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[Profile]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.Profile().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def update(self, reqdata: ProfileUpdateData, ctrl=None) -> Profile:
        utility = self._utility
        ctx = utility.make_context({
            "opname": "update",
            "ctrl": ctrl,
