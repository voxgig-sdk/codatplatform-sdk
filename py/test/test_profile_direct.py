...
class TestProfileDirect:

    def test_should_direct_list_profile(self):
        setup = _profile_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-profile", "live" if setup["live"] else "unit")
        if _skip:
...
def _profile_direct_setup(mockres):
