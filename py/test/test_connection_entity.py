    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set CODATPLATFORM_TEST_CONNECTION_ENTID JSON to run live")
        client = setup["client"]
        ...
        connection_ref01_ent.remove(connection_ref01_match_rm0, None)
        ...
        connection_ref01_list_rt0_result = connection_ref01_ent.list(connection_ref01_match_rt0, None)
        assert isinstance(connection_ref01_list_rt0_result, list)
...
def _connection_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("CODATPLATFORM_TEST_CONNECTION_ENTID"))
    if idmap_resolved is None:
        idmap_resolved = helpers.to_map(idmap)
    if idmap_resolved.get("company_id") is None:
        idmap_resolved["company_id"] = idmap_resolved.get("company01")
