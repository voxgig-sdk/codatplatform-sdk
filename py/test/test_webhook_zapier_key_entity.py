...
class TestWebhookZapierKeyEntity:

    def test_should_create_instance(self):
        testsdk = CodatplatformSDK.test(None, None)
        ent = testsdk.WebhookZapierKey(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _webhook_zapier_key_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["create"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "webhook_zapier_key." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set CODATPLATFORM_TEST_WEBHOOK_ZAPIER_KEY_ENTID JSON to run live")
        client = setup["client"]

        # CREATE
        webhook_zapier_key_ref01_ent = client.WebhookZapierKey(None)
...
def _webhook_zapier_key_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/webhook_zapier_key/WebhookZapierKeyTestData.json")
    ...
    _entid_env_raw = os.environ.get(
        "CODATPLATFORM_TEST_WEBHOOK_ZAPIER_KEY_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("CODATPLATFORM_TEST_WEBHOOK_ZAPIER_KEY_ENTID"))
