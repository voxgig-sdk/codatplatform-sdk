...
class TestRefreshDataEntity:

    def test_should_create_instance(self):
        testsdk = CodatplatformSDK.test(None, None)
        ent = testsdk.RefreshData(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _refresh_data_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["create"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "refresh_data." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set CODATPLATFORM_TEST_REFRESH_DATA_ENTID JSON to run live")
        client = setup["client"]

        # CREATE
        refresh_data_ref01_ent = client.RefreshData(None)
...
def _refresh_data_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/refresh_data/RefreshDataTestData.json")
    ...
    _entid_env_raw = os.environ.get(
        "CODATPLATFORM_TEST_REFRESH_DATA_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("CODATPLATFORM_TEST_REFRESH_DATA_ENTID"))
