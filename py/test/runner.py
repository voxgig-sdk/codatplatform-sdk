...
class CodatplatformTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            CodatplatformTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = CodatplatformTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = CodatplatformTestRunner.getenv("CODATPLATFORM_TEST_LIVE")
        override = CodatplatformTestRunner.getenv("CODATPLATFORM_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = CodatplatformTestRunner.getenv(key)
        ...
        explain = CodatplatformTestRunner.getenv("CODATPLATFORM_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["CODATPLATFORM_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if CodatplatformTestRunner._test_control is not None:
            return CodatplatformTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                CodatplatformTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            CodatplatformTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = CodatplatformTestRunner.load_test_control()
...
def load_env_local():
    CodatplatformTestRunner.load_env_local()


def env_override(m):
    return CodatplatformTestRunner.env_override(m)


def entity_data(v):
    return CodatplatformTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return CodatplatformTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return CodatplatformTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return CodatplatformTestRunner.load_test_control()


def live_client_options():
    return CodatplatformTestRunner.live_client_options()


def live_delay_ms():
    return CodatplatformTestRunner.live_delay_ms()
