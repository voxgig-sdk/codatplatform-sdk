...
class TestValidationEntity:
    ...
    def test_should_stream(self):
        ...
        seen = list(base.Validation(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.Validation(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        setup = _validation_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["list"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "validation." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set CODATPLATFORM_TEST_VALIDATION_ENTID JSON to run live")
        client = setup["client"]

        # Bootstrap entity data from existing test data.
        validation_ref01_data_raw = vs.items(helpers.to_map(
            vs.getpath(setup["data"], "existing.validation")))
        validation_ref01_data = None
        if len(validation_ref01_data_raw) > 0:
            validation_ref01_data = helpers.to_map(validation_ref01_data_raw[0][1])

        # LIST
        validation_ref01_ent = client.Validation(None)
        validation_ref01_match = {
            "company_id": setup["idmap"]["company01"],
            "sync_id": setup["idmap"]["sync01"],
        }

        validation_ref01_list_result = validation_ref01_ent.list(validation_ref01_match, None)
        assert isinstance(validation_ref01_list_result, list)



def _validation_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/validation/ValidationTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("CODATPLATFORM_TEST_VALIDATION_ENTID"))
