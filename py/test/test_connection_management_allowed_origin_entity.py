...
class TestConnectionManagementAllowedOriginEntity:

    def test_should_create_instance(self):
        testsdk = CodatplatformSDK.test(None, None)
        ent = testsdk.ConnectionManagementAllowedOrigin(None)
    ...
    def test_should_stream(self):
        ...
        seen = list(base.ConnectionManagementAllowedOrigin(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.ConnectionManagementAllowedOrigin(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        setup = _connection_management_allowed_origin_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["create", "list"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "connection_management_allowed_origin." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set CODATPLATFORM_TEST_CONNECTION_MANAGEMENT_ALLOWED_ORIGIN_ENTID JSON to run live")
        client = setup["client"]

        # CREATE
        connection_management_allowed_origin_ref01_ent = client.ConnectionManagementAllowedOrigin(None)
        ...
        connection_management_allowed_origin_ref01_list_result = connection_management_allowed_origin_ref01_ent.list(connection_management_allowed_origin_ref01_match, None)
        assert isinstance(connection_management_allowed_origin_ref01_list_result, list)



def _connection_management_allowed_origin_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/connection_management_allowed_origin/ConnectionManagementAllowedOriginTestData.json")
    ...
    _entid_env_raw = os.environ.get(
        "CODATPLATFORM_TEST_CONNECTION_MANAGEMENT_ALLOWED_ORIGIN_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("CODATPLATFORM_TEST_CONNECTION_MANAGEMENT_ALLOWED_ORIGIN_ENTID"))
