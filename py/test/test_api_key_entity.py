    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set CODATPLATFORM_TEST_API_KEY_ENTID JSON to run live")
        client = setup["client"]

        # Bootstrap entity data from existing test data.
        api_key_ref01_data_raw = vs.items(helpers.to_map(
            vs.getpath(setup["data"], "existing.api_key")))
        api_key_ref01_data = None
        if len(api_key_ref01_data_raw) > 0:
...
def _api_key_basic_setup(extra):
    ...
    _entid_env_raw = os.environ.get(
        "CODATPLATFORM_TEST_API_KEY_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("CODATPLATFORM_TEST_API_KEY_ENTID"))
