...
class TestSettingDirect:

    def test_should_direct_list_setting(self):
        setup = _setting_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-setting", "live" if setup["live"] else "unit")
        if _skip:
...
def _setting_direct_setup(mockres):
