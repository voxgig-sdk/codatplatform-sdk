...
class TestSettingEntity:
    ...
    def test_should_stream(self):
        ...
        seen = list(base.Setting(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.Setting(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        setup = _setting_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["create", "list", "remove"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "setting." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set CODATPLATFORM_TEST_SETTING_ENTID JSON to run live")
        client = setup["client"]

        # CREATE
        setting_ref01_ent = client.Setting(None)
        ...
        setting_ref01_list_result = setting_ref01_ent.list(setting_ref01_match, None)
        assert isinstance(setting_ref01_list_result, list)
        ...
        setting_ref01_ent.remove(setting_ref01_match_rm0, None)

        # LIST
        setting_ref01_match_rt0 = {}

        setting_ref01_list_rt0_result = setting_ref01_ent.list(setting_ref01_match_rt0, None)
        assert isinstance(setting_ref01_list_rt0_result, list)
...
def _setting_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/setting/SettingTestData.json")
    ...
    _entid_env_raw = os.environ.get(
        "CODATPLATFORM_TEST_SETTING_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("CODATPLATFORM_TEST_SETTING_ENTID"))
