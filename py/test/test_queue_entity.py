    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set CODATPLATFORM_TEST_QUEUE_ENTID JSON to run live")
        client = setup["client"]

        # Bootstrap entity data from existing test data.
        queue_ref01_data_raw = vs.items(helpers.to_map(
            vs.getpath(setup["data"], "existing.queue")))
        queue_ref01_data = None
        if len(queue_ref01_data_raw) > 0:
...
def _queue_basic_setup(extra):
    ...
    _entid_env_raw = os.environ.get(
        "CODATPLATFORM_TEST_QUEUE_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("CODATPLATFORM_TEST_QUEUE_ENTID"))
