...
class TestSyncSettingDirect:

    def test_should_direct_list_sync_setting(self):
        setup = _sync_setting_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-sync_setting", "live" if setup["live"] else "unit")
        if _skip:
...
def _sync_setting_direct_setup(mockres):
