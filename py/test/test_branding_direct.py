...
class TestBrandingDirect:

    def test_should_direct_load_branding(self):
        setup = _branding_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-branding", "live" if setup["live"] else "unit")
        if _skip:
...
def _branding_direct_setup(mockres):
