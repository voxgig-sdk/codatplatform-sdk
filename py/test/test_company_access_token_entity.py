...
class TestCompanyAccessTokenEntity:

    def test_should_create_instance(self):
        testsdk = CodatplatformSDK.test(None, None)
        ent = testsdk.CompanyAccessToken(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _company_access_token_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "company_access_token." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set CODATPLATFORM_TEST_COMPANY_ACCESS_TOKEN_ENTID JSON to run live")
        client = setup["client"]

        # Bootstrap entity data from existing test data.
        company_access_token_ref01_data_raw = vs.items(helpers.to_map(
            vs.getpath(setup["data"], "existing.company_access_token")))
        company_access_token_ref01_data = None
        if len(company_access_token_ref01_data_raw) > 0:
...
def _company_access_token_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/company_access_token/CompanyAccessTokenTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("CODATPLATFORM_TEST_COMPANY_ACCESS_TOKEN_ENTID"))
