...
class TestDataStatusDirect:

    def test_should_direct_load_data_status(self):
        setup = _data_status_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-data_status", "live" if setup["live"] else "unit")
        if _skip:
...
def _data_status_direct_setup(mockres):
