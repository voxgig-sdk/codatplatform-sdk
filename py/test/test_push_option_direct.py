...
class TestPushOptionDirect:

    def test_should_direct_load_push_option(self):
        setup = _push_option_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-push_option", "live" if setup["live"] else "unit")
        if _skip:
...
def _push_option_direct_setup(mockres):
