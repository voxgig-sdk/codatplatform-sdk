...
module CodatplatformUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = CodatplatformResponse.new({})
    ...
    if fetch_err
      ...
      response = CodatplatformResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = CodatplatformResponse.new(fetched)
    else
