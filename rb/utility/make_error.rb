...
module CodatplatformUtilities
  MakeError = ->(ctx, err) {
    if ctx.nil?
      ...
      ctx = CodatplatformContext.new({}, nil)
    end
    op = ctx.op || CodatplatformOperation.new({})
    opname = op.name
    opname = "unknown operation" if opname.empty? || opname == "_"

    result = ctx.result || CodatplatformResult.new({})
    result.ok = false

    err = result.err if err.nil?
    err = ctx.make_error("unknown", "unknown error") if err.nil?

    errmsg = err.is_a?(CodatplatformError) ? err.msg : err.to_s
    ...
    sdk_err = CodatplatformError.new("", msg, ctx)
    ...
    sdk_err.code = err.code if err.is_a?(CodatplatformError)
