...
class ConnectionManagementAllowedOriginEntity
  def initialize(client, entopts = nil)
    ...
    @_name = "connection_management_allowed_origin"
    @_client = client
  ...
  def make
    opts = @_entopts.dup
    ConnectionManagementAllowedOriginEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = CodatplatformHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [ConnectionManagementAllowedOrigin, Hash] the current ConnectionManagementAllowedOrigin data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = CodatplatformHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of ConnectionManagementAllowedOrigin fields)
  def match_get
  ...
  #
  # @param reqmatch [ConnectionManagementAllowedOriginListMatch, Hash, nil] match filter (any subset of
  #   ConnectionManagementAllowedOrigin fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<ConnectionManagementAllowedOrigin>, Array] the matching ConnectionManagementAllowedOrigin items; raises CodatplatformError on failure
  ...
  #
  # @param reqdata [ConnectionManagementAllowedOriginCreateData, Hash, nil] body data
  # @param ctrl [Object, nil] optional per-call control
  # @return [ConnectionManagementAllowedOrigin, Hash] the created ConnectionManagementAllowedOrigin; raises CodatplatformError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        if ctx.result.resdata
          @_data = CodatplatformHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
