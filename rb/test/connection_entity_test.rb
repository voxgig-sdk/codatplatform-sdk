...
class ConnectionEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set CODATPLATFORM_TEST_CONNECTION_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    connection_ref01_ent.remove(connection_ref01_match_rm0, nil)
    ...
    connection_ref01_list_rt0_result = connection_ref01_ent.list(connection_ref01_match_rt0, nil)
    assert connection_ref01_list_rt0_result.is_a?(Array)
...
def connection_basic_setup(extra)
  ...
  entid_env_raw = ENV["CODATPLATFORM_TEST_CONNECTION_ENTID"]
  ...
  if idmap_resolved["company_id"].nil?
