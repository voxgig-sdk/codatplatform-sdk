...
class IntegrationEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set CODATPLATFORM_TEST_INTEGRATION_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    integration_ref01_data = nil
    if integration_ref01_data_raw.length > 0
...
def integration_basic_setup(extra)
  ...
  entid_env_raw = ENV["CODATPLATFORM_TEST_INTEGRATION_ENTID"]
