...
class SupplementalDataConfigDirectTest < Minitest::Test
  def test_direct_load_supplemental_data_config
    setup = supplemental_data_config_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-supplemental_data_config", setup[:live] ? "live" : "unit")
    if _should_skip
...
def supplemental_data_config_direct_setup(mockres)
