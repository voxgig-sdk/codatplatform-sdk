...
class BrandingDirectTest < Minitest::Test
  def test_direct_load_branding
    setup = branding_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-branding", setup[:live] ? "live" : "unit")
    if _should_skip
...
def branding_direct_setup(mockres)
