...
class ValidationEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    seen = base.Validation(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.Validation(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    setup = validation_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["list"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "validation." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set CODATPLATFORM_TEST_VALIDATION_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    validation_ref01_data = nil
    if validation_ref01_data_raw.length > 0
      validation_ref01_data = Helpers.to_map(validation_ref01_data_raw[0][1])
    end

    # LIST
    validation_ref01_ent = client.Validation(nil)
    validation_ref01_match = {
      "company_id" => setup[:idmap]["company01"],
      "sync_id" => setup[:idmap]["sync01"],
    }

    validation_ref01_list_result = validation_ref01_ent.list(validation_ref01_match, nil)
    assert validation_ref01_list_result.is_a?(Array)

  end
end

def validation_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "validation", "ValidationTestData.json")
  ...
  entid_env_raw = ENV["CODATPLATFORM_TEST_VALIDATION_ENTID"]
