...
class ApiKeyEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set CODATPLATFORM_TEST_API_KEY_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    api_key_ref01_data = nil
    if api_key_ref01_data_raw.length > 0
...
def api_key_basic_setup(extra)
  ...
  entid_env_raw = ENV["CODATPLATFORM_TEST_API_KEY_ENTID"]
