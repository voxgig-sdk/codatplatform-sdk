...
class SettingEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    seen = base.Setting(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.Setting(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    setup = setting_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["create", "list", "remove"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "setting." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set CODATPLATFORM_TEST_SETTING_ENTID JSON to run live"
      return
    end
    client = setup[:client]

    # CREATE
    setting_ref01_ent = client.Setting(nil)
    ...
    setting_ref01_list_result = setting_ref01_ent.list(setting_ref01_match, nil)
    assert setting_ref01_list_result.is_a?(Array)
    ...
    # REMOVE
    setting_ref01_match_rm0 = {
      "id" => setting_ref01_data["id"],
    }
    setting_ref01_ent.remove(setting_ref01_match_rm0, nil)

    # LIST
    setting_ref01_match_rt0 = {}

    setting_ref01_list_rt0_result = setting_ref01_ent.list(setting_ref01_match_rt0, nil)
    assert setting_ref01_list_rt0_result.is_a?(Array)
...
def setting_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "setting", "SettingTestData.json")
  ...
  entid_env_raw = ENV["CODATPLATFORM_TEST_SETTING_ENTID"]
