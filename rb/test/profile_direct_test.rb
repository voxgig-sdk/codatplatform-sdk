...
class ProfileDirectTest < Minitest::Test
  def test_direct_list_profile
    setup = profile_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-profile", setup[:live] ? "live" : "unit")
    if _should_skip
...
def profile_direct_setup(mockres)
