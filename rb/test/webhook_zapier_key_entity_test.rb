...
class WebhookZapierKeyEntityTest < Minitest::Test
  ...
  def test_basic_flow
    setup = webhook_zapier_key_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["create"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "webhook_zapier_key." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set CODATPLATFORM_TEST_WEBHOOK_ZAPIER_KEY_ENTID JSON to run live"
      return
    end
    client = setup[:client]

    # CREATE
    webhook_zapier_key_ref01_ent = client.WebhookZapierKey(nil)
...
def webhook_zapier_key_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "webhook_zapier_key", "WebhookZapierKeyTestData.json")
  ...
  entid_env_raw = ENV["CODATPLATFORM_TEST_WEBHOOK_ZAPIER_KEY_ENTID"]
