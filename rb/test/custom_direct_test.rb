...
class CustomDirectTest < Minitest::Test
  def test_direct_load_custom
    setup = custom_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-custom", setup[:live] ? "live" : "unit")
    if _should_skip
...
def custom_direct_setup(mockres)
