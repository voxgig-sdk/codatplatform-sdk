...
class ConnectionManagementAccessTokenDirectTest < Minitest::Test
  def test_direct_load_connection_management_access_token
    setup = connection_management_access_token_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-connection_management_access_token", setup[:live] ? "live" : "unit")
    if _should_skip
...
def connection_management_access_token_direct_setup(mockres)
