...
class OptionEntityTest < Minitest::Test
  ...
  def test_basic_flow
    setup = option_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    [].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "option." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set CODATPLATFORM_TEST_OPTION_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    option_ref01_data = nil
    if option_ref01_data_raw.length > 0
...
def option_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "option", "OptionTestData.json")
  ...
  entid_env_raw = ENV["CODATPLATFORM_TEST_OPTION_ENTID"]
