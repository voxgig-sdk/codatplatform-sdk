...
(0, node_test_1.describe)('ConnectionDirect', async () => {
    ...
    (0, node_test_1.test)('direct-load-connection', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup({ id: 'direct01' });
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-connection', setup.live))
            return;
        ...
        if (setup.live) {
            ...
            params.id = "2e9d2c44-f675-40ba-8049-353bfcb5e171";
        }
        else {
        ...
        const result = await client.direct({
            path: 'companies/{company_id}/connections/{id}',
            method: 'GET',
            params,
            query,
        });
...
function directSetup(mockres) {
    ...
    const env = (0, utility_1.envOverride)({
        'CODATPLATFORM_TEST_CONNECTION_ENTID': {},
    ...
    if (live) {
        ...
        let idmap = env['CODATPLATFORM_TEST_CONNECTION_ENTID'];
