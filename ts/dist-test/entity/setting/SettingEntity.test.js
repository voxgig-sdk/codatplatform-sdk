...
(0, node_test_1.describe)('SettingEntity', async () => {
    ...
    (0, node_test_1.test)('basic', async (t) => {
        ...
        for (const op of ['create', 'list', 'remove']) {
            if ((0, utility_1.maybeSkipControl)(t, 'entityOp', 'setting.' + op, live))
                return;
        }
...
function basicSetup(extra) {
    ...
    let idmap = transform(['setting01', 'setting02', 'setting03', 'api_key01', 'api_key02', 'api_key03'], {
        '`$PACK`': ['', {
                '`$KEY`': '`$COPY`',
    ...
    const idmapEnvVal = process.env['CODATPLATFORM_TEST_SETTING_ENTID'];
