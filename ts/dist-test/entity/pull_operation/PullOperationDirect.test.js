...
(0, node_test_1.describe)('PullOperationDirect', async () => {
    ...
    (0, node_test_1.test)('direct-exists', async () => {
        const sdk = new __1.CodatplatformSDK({
    ...
    (0, node_test_1.test)('direct-load-pull_operation', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup({ id: 'direct01' });
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-pull_operation', setup.live))
            return;
        if ((0, utility_1.skipIfMissingIds)(t, setup, ["company01", "dataset01"]))
            return;
        ...
        if (setup.live) {
            ...
            const candidateId = listArr[0]?.company_id ?? listArr[0]?.id;
            if (null == candidateId) {
                throw new Error('Live load blocked: discovery returned no usable identity');
            }
            params.company_id = candidateId;
    ...
    (0, node_test_1.test)('direct-list-pull_operation', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }]);
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-list-pull_operation', setup.live))
            return;
        if ((0, utility_1.skipIfMissingIds)(t, setup, ["company01"]))
            return;
        const { client, calls } = setup;
        const params = {};
        const query = {};
...
function liveScenariosActive() { return false && process.env.CODATPLATFORM_TEST_LIVE === 'TRUE'; }
function directSetup(mockres) {
    ...
    if (live) {
        ...
        const client = new __1.CodatplatformSDK(Object.assign({}, (0, utility_1.liveClientOptions)(), { system: { fetch: transport.fetch },
            apikey: env.CODATPLATFORM_APIKEY,
        }));
        let idmap = env['CODATPLATFORM_TEST_PULL_OPERATION_ENTID'];
    ...
    const client = new __1.CodatplatformSDK({
        base: 'http://localhost:8080',
...
//# sourceMappingURL=PullOperationDirect.test.js.map