...
describe('ValidationEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['list']) {
      if (maybeSkipControl(t, 'entityOp', 'validation.' + op, live)) return
    }
    ...
    const validation_ref01_match: any = {}
    validation_ref01_match['company_id'] = setup.idmap['company01']
    validation_ref01_match['sync_id'] = setup.idmap['sync01']

    const validation_ref01_list = (await validation_ref01_ent.list(validation_ref01_match)).map((e: any) => e.data())
