...
describe('CompanyAccessTokenDirect', async () => {
  ...
  test('direct-load-company_access_token', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-company_access_token', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['CODATPLATFORM_TEST_COMPANY_ACCESS_TOKEN_ENTID']
