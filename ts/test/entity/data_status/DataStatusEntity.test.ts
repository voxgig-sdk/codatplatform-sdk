...
describe('DataStatusEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['load']) {
      if (maybeSkipControl(t, 'entityOp', 'data_status.' + op, live)) return
    }
    ...
    let data_status_ref01_data = Object.values(setup.data.existing.data_status)[0] as any
...
function basicSetup(extra?: any) {
  ...
  const entityDataFile =
    Path.resolve(__dirname, 
      '../../../../.sdk/test/entity/data_status/DataStatusTestData.json')
  ...
  const idmapEnvVal = process.env['CODATPLATFORM_TEST_DATA_STATUS_ENTID']
