...
describe('PullOperationDirect', async () => {
  ...
  test('direct-load-pull_operation', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-pull_operation', setup.live)) return
    if (skipIfMissingIds(t, setup, ["company01","dataset01"])) return
  ...
  test('direct-list-pull_operation', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-pull_operation', setup.live)) return
    if (skipIfMissingIds(t, setup, ["company01"])) return
    const { client, calls } = setup
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['CODATPLATFORM_TEST_PULL_OPERATION_ENTID']
