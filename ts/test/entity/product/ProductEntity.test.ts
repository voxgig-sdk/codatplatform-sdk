...
describe('ProductEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of []) {
      if (maybeSkipControl(t, 'entityOp', 'product.' + op, live)) return
    }
    ...
    if (setup.syntheticOnly) {
      t.skip('live entity test uses synthetic IDs from fixture — set CODATPLATFORM_TEST_PRODUCT_ENTID JSON to run live')
      return
    }
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['CODATPLATFORM_TEST_PRODUCT_ENTID']
