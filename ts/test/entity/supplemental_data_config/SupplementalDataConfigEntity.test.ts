...
describe('SupplementalDataConfigEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['load']) {
      if (maybeSkipControl(t, 'entityOp', 'supplemental_data_config.' + op, live)) return
    }
    ...
    let supplemental_data_config_ref01_data = Object.values(setup.data.existing.supplemental_data_config)[0] as any
...
function basicSetup(extra?: any) {
  ...
  let idmap = transform(
    ['supplemental_data_config01','supplemental_data_config02','supplemental_data_config03','integration01','integration02','integration03','data_type01','data_type02','data_type03'],
    {
      '`$PACK`': ['', {
        '`$KEY`': '`$COPY`',
  ...
  const idmapEnvVal = process.env['CODATPLATFORM_TEST_SUPPLEMENTAL_DATA_CONFIG_ENTID']
