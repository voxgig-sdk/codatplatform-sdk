...
describe('SupplementalDataConfigDirect', async () => {
  ...
  test('direct-load-supplemental_data_config', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-supplemental_data_config', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['CODATPLATFORM_TEST_SUPPLEMENTAL_DATA_CONFIG_ENTID']
