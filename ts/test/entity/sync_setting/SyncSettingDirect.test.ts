...
describe('SyncSettingDirect', async () => {
  ...
  test('direct-list-sync_setting', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-sync_setting', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['CODATPLATFORM_TEST_SYNC_SETTING_ENTID']
