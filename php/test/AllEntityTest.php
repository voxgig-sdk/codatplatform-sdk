...
class AllEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set CODATPLATFORM_TEST_ALL_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function all_basic_setup($extra)
{
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["all01", "all02", "all03", "company01", "company02", "company03"] as $k) {
    ...
    $entid_env_raw = getenv("CODATPLATFORM_TEST_ALL_ENTID");
    ...
    $idmap_resolved = Helpers::to_map(
        $env["CODATPLATFORM_TEST_ALL_ENTID"]);
