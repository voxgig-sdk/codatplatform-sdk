...
class IntegrationEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set CODATPLATFORM_TEST_INTEGRATION_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $integration_ref01_data = null;
        if (count($integration_ref01_data_raw) > 0) {
...
function integration_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("CODATPLATFORM_TEST_INTEGRATION_ENTID");
