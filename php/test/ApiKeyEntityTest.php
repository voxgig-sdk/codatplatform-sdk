...
class ApiKeyEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set CODATPLATFORM_TEST_API_KEY_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];

        // Bootstrap entity data from existing test data.
        $api_key_ref01_data_raw = Vs::items(Helpers::to_map(
            Vs::getpath($setup["data"], "existing.api_key")));
        $api_key_ref01_data = null;
        if (count($api_key_ref01_data_raw) > 0) {
            $api_key_ref01_data = Helpers::to_map($api_key_ref01_data_raw[0][1]);
        }
...
function api_key_basic_setup($extra)
{
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["api_key01", "api_key02", "api_key03"] as $k) {
    ...
    $entid_env_raw = getenv("CODATPLATFORM_TEST_API_KEY_ENTID");
