...
class HistoryEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set CODATPLATFORM_TEST_HISTORY_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $history_ref01_data = null;
        if (count($history_ref01_data_raw) > 0) {
...
function history_basic_setup($extra)
{
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["history01", "history02", "history03", "company01", "company02", "company03"] as $k) {
    ...
    $entid_env_raw = getenv("CODATPLATFORM_TEST_HISTORY_ENTID");
