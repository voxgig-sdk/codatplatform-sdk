...
class SupplementalDataEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = CodatplatformSDK::test(null, null);
        $ent = $testsdk->SupplementalData(null);
        $this->assertNotNull($ent);
    }

    public function test_basic_flow(): void
    {
        $setup = supplemental_data_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["update"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "supplemental_data." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set CODATPLATFORM_TEST_SUPPLEMENTAL_DATA_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $supplemental_data_ref01_data = null;
        if (count($supplemental_data_ref01_data_raw) > 0) {
            $supplemental_data_ref01_data = Helpers::to_map($supplemental_data_ref01_data_raw[0][1]);
        }

        // UPDATE
        $supplemental_data_ref01_ent = $client->SupplementalData(null);
...
function supplemental_data_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/supplemental_data/SupplementalDataTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["supplemental_data01", "supplemental_data02", "supplemental_data03", "integration01", "integration02", "integration03", "data_type01", "data_type02", "data_type03", "platform_key01"] as $k) {
    ...
    $entid_env_raw = getenv("CODATPLATFORM_TEST_SUPPLEMENTAL_DATA_ENTID");
