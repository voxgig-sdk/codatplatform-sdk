...
class SettingEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set CODATPLATFORM_TEST_SETTING_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $setting_ref01_ent->remove($setting_ref01_match_rm0, null);

        // LIST
        $setting_ref01_match_rt0 = [];

        $setting_ref01_list_rt0_result = $setting_ref01_ent->list($setting_ref01_match_rt0, null);
        $this->assertIsArray($setting_ref01_list_rt0_result);
...
function setting_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("CODATPLATFORM_TEST_SETTING_ENTID");
