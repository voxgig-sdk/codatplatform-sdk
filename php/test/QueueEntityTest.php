...
class QueueEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set CODATPLATFORM_TEST_QUEUE_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $queue_ref01_data = null;
        if (count($queue_ref01_data_raw) > 0) {
...
function queue_basic_setup($extra)
{
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["queue01", "queue02", "queue03", "company01", "company02", "company03"] as $k) {
    ...
    $entid_env_raw = getenv("CODATPLATFORM_TEST_QUEUE_ENTID");
