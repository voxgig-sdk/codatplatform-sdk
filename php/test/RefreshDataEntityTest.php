...
class RefreshDataEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = CodatplatformSDK::test(null, null);
        $ent = $testsdk->RefreshData(null);
        $this->assertNotNull($ent);
    }

    public function test_basic_flow(): void
    {
        $setup = refresh_data_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["create"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "refresh_data." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set CODATPLATFORM_TEST_REFRESH_DATA_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];

        // CREATE
        $refresh_data_ref01_ent = $client->RefreshData(null);
...
function refresh_data_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/refresh_data/RefreshDataTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["refresh_data01", "refresh_data02", "refresh_data03", "company01", "company02", "company03"] as $k) {
    ...
    $entid_env_raw = getenv("CODATPLATFORM_TEST_REFRESH_DATA_ENTID");
