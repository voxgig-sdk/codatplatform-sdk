...
class ValidationEntityTest extends TestCase
{
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->Validation(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->Validation(null)->stream("list", null, null) as $item) {
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set CODATPLATFORM_TEST_VALIDATION_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $validation_ref01_list_result = $validation_ref01_ent->list($validation_ref01_match, null);
        $this->assertIsArray($validation_ref01_list_result);
...
function validation_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("CODATPLATFORM_TEST_VALIDATION_ENTID");
