...
class PushEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = CodatplatformSDK::test(null, null);
        $ent = $testsdk->Push(null);
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->Push(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->Push(null)->stream("list", null, null) as $item) {
    ...
    {
        $setup = push_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["list", "load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "push." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set CODATPLATFORM_TEST_PUSH_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $push_ref01_data = null;
        if (count($push_ref01_data_raw) > 0) {
            $push_ref01_data = Helpers::to_map($push_ref01_data_raw[0][1]);
        }

        // LIST
        $push_ref01_ent = $client->Push(null);
        $push_ref01_match = [
            "company_id" => $setup["idmap"]["company01"],
        ];

        $push_ref01_list_result = $push_ref01_ent->list($push_ref01_match, null);
...
function push_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/push/PushTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["push01", "push02", "push03", "company01", "company02", "company03"] as $k) {
    ...
    $entid_env_raw = getenv("CODATPLATFORM_TEST_PUSH_ENTID");
