...
class CompanyAccessTokenEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = CodatplatformSDK::test(null, null);
        $ent = $testsdk->CompanyAccessToken(null);
        $this->assertNotNull($ent);
    }

    public function test_basic_flow(): void
    {
        $setup = company_access_token_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "company_access_token." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set CODATPLATFORM_TEST_COMPANY_ACCESS_TOKEN_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $company_access_token_ref01_data = null;
        if (count($company_access_token_ref01_data_raw) > 0) {
...
function company_access_token_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/company_access_token/CompanyAccessTokenTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["company_access_token01", "company_access_token02", "company_access_token03"] as $k) {
    ...
    $entid_env_raw = getenv("CODATPLATFORM_TEST_COMPANY_ACCESS_TOKEN_ENTID");
