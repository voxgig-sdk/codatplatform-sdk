...
class OptionEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set CODATPLATFORM_TEST_OPTION_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $option_ref01_data = null;
        if (count($option_ref01_data_raw) > 0) {
...
function option_basic_setup($extra)
{
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["option01", "option02", "option03", "company01", "company02", "company03", "connection01", "connection02", "connection03"] as $k) {
    ...
    $entid_env_raw = getenv("CODATPLATFORM_TEST_OPTION_ENTID");
