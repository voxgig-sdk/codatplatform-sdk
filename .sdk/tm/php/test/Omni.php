...
//    only - a typed CodatplatformContext there would make every ctx
...
final class CodatplatformOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new CodatplatformContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new CodatplatformSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new CodatplatformResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new CodatplatformError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new CodatplatformResponse($clean['response']);
    ...
    public static function err_from_map($m): ?CodatplatformError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new CodatplatformError(is_string($code) ? $code : '', $msg);
    }
