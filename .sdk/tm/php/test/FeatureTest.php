...
// Codatplatform SDK feature test
//
...
class FtCtrl extends CodatplatformControl
{
...
class FtClient
{
    ...
    private CodatplatformUtility $utility;

    public function __construct(CodatplatformUtility $utility, array $options)
    {
    ...
    public function get_utility(): CodatplatformUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (CodatplatformContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public CodatplatformContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (CodatplatformContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = CodatplatformConfig::make_feature($name);
    ...
    private static function buildUrl(CodatplatformSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof CodatplatformError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new CodatplatformResponse($fetched);
        }
    ...
    private function fail(CodatplatformContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (CodatplatformContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
