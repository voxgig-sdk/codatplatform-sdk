...
// Codatplatform SDK retry feature
...
class CodatplatformRetryFeature extends CodatplatformBaseFeature
{
    ...
    public function init(CodatplatformContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (CodatplatformContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(CodatplatformContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(CodatplatformContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
